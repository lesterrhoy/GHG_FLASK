...
def emu_dashboard():
    ...
    try:
        ...
        # Queries for various emission data
        queries = [
    ...
    # Forecast function using Prophet with a 2-month future forecast
    ...
    # Forecast data with R² scores using Prophet for a 2-month forecast
    forecast_periods = 14  # 12 months of 2025 + 2 future months
    forecast_data = {
        "electricity_forecast": {
            "prophet": {
                "forecast": forecast_prophet(electricity_data, periods=forecast_periods)[0],
                "r2_score": forecast_prophet(electricity_data, periods=12)[1],  # For historical R² calculation
            }
        },
        "fuel_forecast": {
            "prophet": {
                "forecast": forecast_prophet(fuel_data, periods=forecast_periods)[0],
                "r2_score": forecast_prophet(fuel_data, periods=12)[1],
            }
        },
        "waste_segregated_forecast": {
            "prophet": {
                "forecast": forecast_prophet(waste_segregated_data, periods=forecast_periods)[0],
                "r2_score": forecast_prophet(waste_segregated_data, periods=12)[1],
            }
        },
        "waste_unsegregated_forecast": {
            "prophet": {
                "forecast": forecast_prophet(waste_unsegregated_data, periods=forecast_periods)[0],
                "r2_score": forecast_prophet(waste_unsegregated_data, periods=12)[1],
            }
        },
        "water_forecast": {
            "prophet": {
                "forecast": forecast_prophet(water_data, periods=forecast_periods)[0],
                "r2_score": forecast_prophet(water_data, periods=12)[1],
            }
        },
        "treated_water_forecast": {
            "prophet": {
                "forecast": forecast_prophet(treated_water_data, periods=forecast_periods)[0],
                "r2_score": forecast_prophet(treated_water_data, periods=12)[1],
            }
        },
    }

    # Emit real-time data for line graphs only if data is present
    if electricity_data and forecast_data:
        ...
        # Emit forecast data for real-time line graph updates
        socketio.emit('update_forecast', {
            "electricity_forecast": forecast_data["electricity_forecast"]["prophet"]["forecast"],
            "fuel_forecast": forecast_data["fuel_forecast"]["prophet"]["forecast"],
            "waste_segregated_forecast": forecast_data["waste_segregated_forecast"]["prophet"]["forecast"],
            "waste_unsegregated_forecast": forecast_data["waste_unsegregated_forecast"]["prophet"]["forecast"],
            "water_forecast": forecast_data["water_forecast"]["prophet"]["forecast"],
            "treated_water_forecast": forecast_data["treated_water_forecast"]["prophet"]["forecast"],
        })
    ...
    # Month labels including future months
    ...
    # Create month labels for the graph
    labels = get_month_labels(0, selected_year, 14)  # 12 months + 2 future months
    ...
    return render_template(
        ...
        labels=labels,
        month_labels=labels  # Use the same labels for both `labels` and `month_labels`
    )
...
def csd_dashboard():
    ...
    try:
        ...
        # Fetch data for LPG
        lpg_data = fetch_data(
            "SELECT Month, YearTransact, GHGEmissionKGCO2e FROM tbllpg "
    ...
    def forecast_with_prophet(data, start_date, periods=12):
        if len(data) > 1:
            try:
                ...
                df = df[df['y'] > 0]  # Ensure no zero values
                model = Prophet(yearly_seasonality=True)
                model.fit(df)
                future = model.make_future_dataframe(periods=periods, freq='M')
                forecast = model.predict(future)
    ...
    # Prepare the actual and forecasted data for all categories
    ...
    # Fuel
    actual_fuel_values = [row['total_emission'] for row in fuel_emissions_data if row['total_emission'] is not None]
    fuel_months = [row['date'].strftime('%m/%Y') for row in fuel_emissions_data]
    fuel_forecasted_values, actual_fuel_values_filtered, fuel_forecast_months = forecast_with_prophet(
        actual_fuel_values, start_date='2020-01-01', periods=12
    )

    # Water
    actual_water_values = [row['FactorKGCO2e'] for row in water_data if row['FactorKGCO2e'] is not None]
    water_months = [row['Date'].strftime('%m/%Y') for row in water_data]
    water_forecasted_values, actual_water_values_filtered, water_forecast_months = forecast_with_prophet(
    ...
    lpg_months = [f"{row['Month']}/{row['YearTransact']}" for row in lpg_data]
    ...
    # Extract months and actual GHG values for Solid Waste Unsegregated
    solid_waste_months = [f"{row['Month']}/{row['Year']}" for row in solid_waste_unsegregated_data]
    actual_solid_waste_values = [row['GHGEmissionKGCO2e'] for row in solid_waste_unsegregated_data if row['GHGEmissionKGCO2e'] is not None]

    # Forecasting Solid Waste Unsegregated
    solid_waste_forecasted_values, actual_solid_waste_values_filtered, solid_waste_forecast_months = forecast_with_prophet(
        actual_solid_waste_values, start_date='2020-01-01', periods=12
    )
    # Extract months and actual GHG values for Solid Waste Segregated
    ...
    # Forecasting Solid Waste Segregated
    ...
    # Extract months and actual GHG values for Accommodation
    ...
    # Extract years and actual GHG values for Flight
    ...
    # Replace None and NaN values for all datasets
    def clean_values(values):
        return [0 if x is None or (isinstance(x, (float, int)) and np.isnan(x)) else x for x in values]

    actual_electricity_values_filtered = clean_values(actual_electricity_values_filtered)
    electricity_forecasted_values = clean_values(electricity_forecasted_values)

    actual_fuel_values_filtered = clean_values(actual_fuel_values_filtered)
    fuel_forecasted_values = clean_values(fuel_forecasted_values)

    actual_water_values_filtered = clean_values(actual_water_values_filtered)
    water_forecasted_values = clean_values(water_forecasted_values)

    actual_treated_water_values_filtered = clean_values(actual_treated_water_values_filtered)
    treated_water_forecasted_values = clean_values(treated_water_forecasted_values)

    actual_lpg_values_filtered = clean_values(actual_lpg_values_filtered)
    lpg_forecasted_values = clean_values(lpg_forecasted_values)

    actual_food_waste_values_filtered = clean_values(actual_food_waste_values_filtered)
    food_waste_forecasted_values = clean_values(food_waste_forecasted_values)

    actual_solid_waste_values_filtered = clean_values(actual_solid_waste_values_filtered)
    solid_waste_forecasted_values = clean_values(solid_waste_forecasted_values)

    # Replace None and NaN values for segregated solid waste
    actual_solid_waste_segregated_values_filtered = clean_values(actual_solid_waste_segregated_values_filtered)
    solid_waste_segregated_forecasted_values = clean_values(solid_waste_segregated_forecasted_values)

    # Replace None and NaN values for accommodation
    actual_accommodation_values_filtered = clean_values(actual_accommodation_values_filtered)
    accommodation_forecasted_values = clean_values(accommodation_forecasted_values)

    actual_flight_values_filtered = clean_values(actual_flight_values_filtered)
    flight_forecasted_values = clean_values(flight_forecasted_values)

    # Function to calculate R² score
    def calculate_r2(actual, forecasted):
        if len(actual) == len(forecasted):
            actual = [float(a) for a in actual]  # Convert Decimal to float
            forecasted = [float(f) for f in forecasted]  # Convert Decimal to float
            actual_mean = sum(actual) / len(actual)
            ss_total = sum((x - actual_mean) ** 2 for x in actual)
            ss_residual = sum((a - f) ** 2 for a, f in zip(actual, forecasted))
            r2_score = 1 - (ss_residual / ss_total) if ss_total != 0 else 0
            return r2_score
        else:
            return None  # R² score cannot be computed if the lists don't match


    # Calculate R² scores
    r2_electricity = calculate_r2(actual_electricity_values_filtered, electricity_forecasted_values) or 0
    r2_fuel = calculate_r2(actual_fuel_values_filtered, fuel_forecasted_values) or 0
    r2_water = calculate_r2(actual_water_values_filtered, water_forecasted_values) or 0
    r2_treated_water = calculate_r2(actual_treated_water_values_filtered, treated_water_forecasted_values) or 0
    r2_lpg = calculate_r2(actual_lpg_values_filtered, lpg_forecasted_values) or 0
    r2_food_waste = calculate_r2(actual_food_waste_values_filtered, food_waste_forecasted_values) or 0
    r2_solid_waste = calculate_r2(actual_solid_waste_values_filtered, solid_waste_forecasted_values) or 0
    r2_solid_waste_segregated = calculate_r2(actual_solid_waste_segregated_values_filtered, solid_waste_segregated_forecasted_values) or 0
    r2_accommodation = calculate_r2(actual_accommodation_values_filtered, accommodation_forecasted_values) or 0
    r2_flight = calculate_r2(actual_flight_values_filtered, flight_forecasted_values) or 0


    # Emit real-time data to the client with safe defaults
    def safe_emit_data(data):
        for key, value in data.items():
            if isinstance(value, Decimal):
                data[key] = float(value)  # Convert Decimal to float
            elif isinstance(value, list):
                data[key] = [float(item) if isinstance(item, Decimal) else item for item in value]
            elif value is None:
                data[key] = 0  # Replace None with 0 or an appropriate default value
        return data

    # Prepare safe data for all emissions
    safe_electricity_data = safe_emit_data({
        'actual_values': actual_electricity_values_filtered,
        'forecasted_values': electricity_forecasted_values,
        'months': electricity_months,
        'forecast_months': electricity_forecast_months,
        'r2_score': r2_electricity
    })

    safe_fuel_data = safe_emit_data({
        'actual_values': actual_fuel_values_filtered,
        'forecasted_values': fuel_forecasted_values,
        'months': fuel_months,
        'forecast_months': fuel_forecast_months,
        'r2_score': r2_fuel
    })

    safe_water_data = safe_emit_data({
        'actual_values': actual_water_values_filtered,
        'forecasted_values': water_forecasted_values,
        'months': water_months,
        'forecast_months': water_forecast_months,
        'r2_score': r2_water
    })

    safe_treated_water_data = safe_emit_data({
        'actual_values': actual_treated_water_values_filtered,
        'forecasted_values': treated_water_forecasted_values,
        'months': treated_water_months,
        'forecast_months': treated_water_forecast_months,
        'r2_score': r2_treated_water
    })

    safe_lpg_data = safe_emit_data({
        'actual_values': actual_lpg_values_filtered,
        'forecasted_values': lpg_forecasted_values,
        'months': lpg_months,
        'forecast_months': lpg_forecast_months,
        'r2_score': r2_lpg
    })

    safe_food_waste_data = safe_emit_data({
        'actual_values': actual_food_waste_values_filtered,
        'forecasted_values': food_waste_forecasted_values,
        'months': food_waste_months,
        'forecast_months': food_waste_forecast_months,
        'r2_score': r2_food_waste
    })
    # Prepare safe data for solid waste unsegregated
    safe_solid_waste_data = safe_emit_data({
        'actual_values': actual_solid_waste_values_filtered,
        'forecasted_values': solid_waste_forecasted_values,
        'months': solid_waste_months,
        'forecast_months': solid_waste_forecast_months,
        'r2_score': r2_solid_waste
    })
    # Prepare safe data for solid waste segregated
    safe_solid_waste_segregated_data = safe_emit_data({
        'actual_values': actual_solid_waste_segregated_values_filtered,
        'forecasted_values': solid_waste_segregated_forecasted_values,
        'months': solid_waste_segregated_months,
        'forecast_months': solid_waste_segregated_forecast_months,
        'r2_score': r2_solid_waste_segregated
    })
    # Prepare safe data for accommodation emissions
    safe_accommodation_data = safe_emit_data({
        'actual_values': actual_accommodation_values_filtered,
        'forecasted_values': accommodation_forecasted_values,
        'years': accommodation_years,
        'forecast_years': accommodation_forecast_years,
        'r2_score': r2_accommodation
    })
    # Prepare safe data for flight emissions
    safe_flight_data = safe_emit_data({
        'actual_values': actual_flight_values_filtered,
        'forecasted_values': flight_forecasted_values,
        'years': flight_years,
        'forecast_years': flight_forecast_years,
        'r2_score': r2_flight
    })
    # Emit data to the client
    socketio.emit('update_electricity_data', safe_electricity_data)
    socketio.emit('update_fuel_data', safe_fuel_data)
    socketio.emit('update_water_data', safe_water_data)
    socketio.emit('update_treated_water_data', safe_treated_water_data)
    socketio.emit('update_lpg_data', safe_lpg_data)
    socketio.emit('update_food_waste_data', safe_food_waste_data)
    socketio.emit('update_solid_waste_data', safe_solid_waste_data)
    socketio.emit('update_solid_waste_segregated_data', safe_solid_waste_segregated_data)
    socketio.emit('update_accommodation_data', safe_accommodation_data)
    socketio.emit('update_flight_data', safe_flight_data)



    # Render the dashboard with actual and forecasted data for all categories
    return render_template(
        'csd_dashboard.html',
        ...
        r2_electricity=r2_electricity,
        actual_fuel_values=actual_fuel_values_filtered,
        fuel_forecasted_values=fuel_forecasted_values,
        fuel_months=fuel_months,
        fuel_forecast_months=fuel_forecast_months,
        r2_fuel=r2_fuel,
        actual_water_values=actual_water_values_filtered,
        water_forecasted_values=water_forecasted_values,
        water_months=water_months,
        water_forecast_months=water_forecast_months,
        r2_water=r2_water,
        ...
        r2_treated_water=r2_treated_water,
        ...
        r2_food_waste=r2_food_waste,
        solid_waste_unsegregated_months=solid_waste_months,
        actual_solid_waste_unsegregated_values=actual_solid_waste_values,
        solid_waste_unsegregated_forecasted_values=solid_waste_forecasted_values,
        solid_waste_unsegregated_forecast_months=solid_waste_forecast_months,
        r2_solid_waste=r2_solid_waste,
        ...
        r2_solid_waste_segregated=r2_solid_waste_segregated,
        ...
        r2_accommodation=r2_accommodation,
...
# Function to fetch and prepare chart data for a specific year and type
def get_updated_chart_data(year, chart_type):
    query_map = {
        'electricity': "SELECT month, year, kg_co2_per_kwh as value FROM electricity_consumption WHERE year = %s ORDER BY month ASC",
        'fuel': "SELECT DATE_FORMAT(date, '%m') as month, YEAR(date) as year, total_emission as value FROM fuel_emissions WHERE YEAR(date) = %s ORDER BY date ASC",
        'water': "SELECT DATE_FORMAT(Date, '%m') as month, YEAR(Date) as year, FactorKGCO2e as value FROM tblwater WHERE YEAR(Date) = %s ORDER BY Date ASC",
        'treated_water': """SELECT MONTH(Month) as month, YEAR(Month) as year, SUM(FactorKGCO2e) as value 
                           FROM tbltreatedwater WHERE YEAR(Month) = %s GROUP BY YEAR(Month), MONTH(Month) ORDER BY YEAR(Month), MONTH(Month)""",
        'lpg': "SELECT Month as month, YearTransact as year, GHGEmissionKGCO2e as value FROM tbllpg WHERE YearTransact = %s ORDER BY YearTransact, Month ASC",
        'food': "SELECT Month as month, YearTransaction as year, GHGEmissionKGCO2e as value FROM tblfoodwaste WHERE YearTransaction = %s ORDER BY YearTransaction, Month ASC",
        'solid_waste_unsegregated': "SELECT Month as month, Year as year, GHGEmissionKGCO2e as value FROM tblsolidwasteunsegregated WHERE Year = %s ORDER BY Year, Month ASC",
        'solid_waste_segregated': """SELECT Month as month, Year as year, SUM(GHGEmissionKGCO2e) as value 
                                     FROM tblsolidwastesegregated 
                                     WHERE Year = %s 
                                     GROUP BY Year, Month 
                                     ORDER BY Year, FIELD(Month, 'January', 'February', 'March', 'April', 
                                     'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')""",
        'accommodation': """SELECT YearTransact as year, SUM(GHGEmissionKGC02e) as value
                            FROM tblaccommodation
                            WHERE YearTransact = %s
                            GROUP BY YearTransact
                            ORDER BY YearTransact ASC""",
        'flight': """SELECT Year as year, SUM(GHGEmissionKGC02e) as value
                     FROM tblflight
                     WHERE Year = %s
                     GROUP BY Year
                     ORDER BY Year ASC"""
    ...
    data = fetch_data_from_db(query, (year,))
    if not data:
        return {'error': f'No data found for {chart_type} in {year}.'}

    # Prepare data for the chart
    actual_values = [row['value'] for row in data if row['value'] is not None]
    months = [f"{int(row['month']):02d}/{row['year']}" for row in data]

    # Perform forecasting with Prophet if data is sufficient
    if len(actual_values) > 1:
        try:
            df = pd.DataFrame({
                'ds': pd.date_range(start=f'{year}-01-01', periods=len(actual_values), freq='M'),
                'y': actual_values
            })
            df = df[df['y'] > 0]  # Filter out zero or negative values
            model = Prophet(yearly_seasonality=True)
            model.fit(df)
            future = model.make_future_dataframe(periods=12, freq='M')
            forecast = model.predict(future)

            forecasted_values = forecast['yhat'][-12:].tolist()
            forecast_months = forecast['ds'][-12:].dt.strftime('%m/%Y').tolist()
        except Exception as e:
            print(f"Prophet forecasting error: {e}")
            forecasted_values = [0] * 12
            forecast_months = [f'01/{year + 1}'] * 12
    else:
        forecasted_values = [0] * 12
        forecast_months = [f'01/{year + 1}'] * 12
...
@socketio.on('request_yearly_data')
def handle_request_yearly_data(data):
    year = int(data['year'])
    chart_type = data['type']

    # Fetch and prepare updated data
    updated_data = get_updated_chart_data(year, chart_type)

    if updated_data:
        socketio.emit(f'update_{chart_type}_data', updated_data)
    else:
        socketio.emit(f'update_{chart_type}_data', {'error': f'No data found for {chart_type} in {year}.'})

# Add a route for real-time updates (this can be triggered by an external source or background task)
@socketio.on('new_data_entry')
def handle_new_data_entry(data):
    """Handles new data entry for real-time updates."""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()

        # Insert new data into the appropriate table based on type
        if data['type'] == 'electricity':
            cursor.execute(
                "INSERT INTO electricity_consumption (month, year, kg_co2_per_kwh) VALUES (%s, %s, %s)",
                (datetime.now().month, datetime.now().year, data['new_value'])
            )
        elif data['type'] == 'fuel':
            cursor.execute(
                "INSERT INTO fuel_emissions (date, total_emission) VALUES (%s, %s)",
                (datetime.now().strftime('%Y-%m-%d'), data['new_value'])
            )
        elif data['type'] == 'water':
            cursor.execute(
                "INSERT INTO tblwater (Date, FactorKGCO2e) VALUES (%s, %s)",
                (datetime.now().strftime('%Y-%m-%d'), data['new_value'])
            )
        elif data['type'] == 'treated_water':
            cursor.execute(
                "INSERT INTO tbltreatedwater (Month, FactorKGCO2e) VALUES (%s, %s)",
                (datetime.now().strftime('%Y-%m-01'), data['new_value'])  # Insert for the first day of the current month
            )
        elif data['type'] == 'lpg':
            cursor.execute(
                "INSERT INTO tbllpg (Month, YearTransact, GHGEmissionKGCO2e) VALUES (%s, %s, %s)",
                (datetime.now().month, datetime.now().year, data['new_value'])
            )
        elif data['type'] == 'food':
            cursor.execute(
                "INSERT INTO tblfoodwaste (Month, YearTransaction, GHGEmissionKGCO2e) VALUES (%s, %s, %s)",
                (datetime.now().month, datetime.now().year, data['new_value'])
            )
        elif data['type'] == 'solid_waste_unsegregated':
            cursor.execute(
                "INSERT INTO tblsolidwasteunsegregated (Month, Year, GHGEmissionKGCO2e) VALUES (%s, %s, %s)",
                (datetime.now().month, datetime.now().year, data['new_value'])
            )
        elif data['type'] == 'solid_waste_segregated':
            cursor.execute(
                "INSERT INTO tblsolidwastesegregated (Month, Year, GHGEmissionKGCO2e) VALUES (%s, %s, %s)",
                (datetime.now().strftime('%B'), datetime.now().year, data['new_value'])
            )
        elif data['type'] == 'accommodation':
            cursor.execute(
                "INSERT INTO tblaccommodation (YearTransact, GHGEmissionKGC02e) VALUES (%s, %s)",
                (datetime.now().year, data['new_value'])
            )
        elif data['type'] == 'flight':
            cursor.execute(
                "INSERT INTO tblflight (Year, GHGEmissionKGC02e) VALUES (%s, %s)",
                (datetime.now().year, data['new_value'])
            )
        else:
            socketio.emit('new_data_confirmed', {'status': 'error', 'message': f"Unsupported data type: {data['type']}"})
            return

        conn.commit()
        socketio.emit('new_data_confirmed', {'status': 'success', 'message': 'New data added successfully'})
    except Exception as e:
        socketio.emit('new_data_confirmed', {'status': 'error', 'message': f"Error: {e}"})
    finally:
        if cursor:
            cursor.close()
        if conn:
...
def clean_for_json(data):
    if isinstance(data, dict):
        ...
        return 0  # Replace None with 0 or a suitable placeholder
    else:
        return data
...
def sdo_dashboard():
    ...
    # Forecast function using Prophet with a 2-month future forecast for monthly data and 1-year future for yearly data
    def forecast_prophet(data, periods, freq='M'):
        if all(v == 0 for v in data):
            return [0] * periods, 0
    ...
    forecast_data = {
        "electricity_forecast": {
            "prophet": {
                "forecast": forecast_prophet(electricity_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(electricity_data, periods=12)[1],
            }
        },
        "fuel_forecast": {
            "prophet": {
                "forecast": forecast_prophet(fuel_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(fuel_data, periods=12)[1],
            }
        },
        "waste_segregated_forecast": {
            "prophet": {
                "forecast": forecast_prophet(waste_segregated_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(waste_segregated_data, periods=12)[1],
            }
        },
        "waste_unsegregated_forecast": {
            "prophet": {
                "forecast": forecast_prophet(waste_unsegregated_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(waste_unsegregated_data, periods=12)[1],
            }
        },
        "water_forecast": {
            "prophet": {
                "forecast": forecast_prophet(water_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(water_data, periods=12)[1],
            }
        },
        "treated_water_forecast": {
            "prophet": {
                "forecast": forecast_prophet(treated_water_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(treated_water_data, periods=12)[1],
            }
        },
        "food_waste_forecast": {
            "prophet": {
                "forecast": forecast_prophet(food_waste_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(food_waste_data, periods=12)[1],
            }
        },
        "lpg_forecast": {
            "prophet": {
                "forecast": forecast_prophet(lpg_data, periods=14)[0],  # 12 months + 2 future months
                "r2_score": forecast_prophet(lpg_data, periods=12)[1],
            }
        },
        "flight_forecast": {
            "prophet": {
                "forecast": forecast_prophet(flight_data, periods=6, freq='Y')[0],  # 5 years + 1 future year
                "r2_score": forecast_prophet(flight_data, periods=5, freq='Y')[1],
            }
        },
        "accommodation_forecast": {
            "prophet": {
                "forecast": forecast_prophet(accommodation_data, periods=6, freq='Y')[0],  # 5 years + 1 future year
...
def procurement_dashboard():
    ...
    def forecast_prophet(data, periods):
        ...
        try:
            ...
            # Make future predictions (12 months + 2 future months)
            future = model.make_future_dataframe(periods=periods, freq='M')
            forecast = model.predict(future)
    ...
    # Forecast data with R² scores (12 months + 2 future months)
    forecast_periods = 14  # 12 months of the current year + 2 future months
    forecast_data = {
        "food_waste_forecast": {
            "prophet": {
                "forecast": forecast_prophet(food_waste_data, periods=forecast_periods)[0],  # Forecast for 14 months
                "r2_score": forecast_prophet(food_waste_data, periods=12)[1],   # R2 score
            }
        },
        "lpg_forecast": {
            "prophet": {
                "forecast": forecast_prophet(lpg_data, periods=forecast_periods)[0],  # Forecast for 14 months
    ...
    labels = get_month_labels(0, selected_year, forecast_periods)  # 12 months + 2 future months
