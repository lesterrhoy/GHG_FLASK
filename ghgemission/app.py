...
socketio = SocketIO(app) # Use a strong secret key in production!
...
def emu_dashboard():
    ...
    try:
        ...
        treated_water_query = """
            ...
            WHERE Campus = %s AND YEAR(Date) = %s
        """
...
def csd_dashboard():
    ...
    # Replace None and NaN values
    actual_electricity_values_filtered = [0 if x is None or np.isnan(x) else x for x in actual_electricity_values_filtered]
    electricity_forecasted_values = [0 if x is None or np.isnan(x) else x for x in electricity_forecasted_values]

    # Calculate R² score for electricity
    r2_electricity = calculate_r2(actual_electricity_values_filtered, electricity_forecasted_values)
    r2_electricity = r2_electricity if r2_electricity is not None else 0  # Ensure R² is not None

    # Emit real-time data to the client with safe defaults
    def safe_emit_data(data):
        """Ensure no None values are present in the data to emit."""
        for key, value in data.items():
            if value is None:
                data[key] = 0  # Replace None with 0 or an appropriate default value
        return data

    safe_data = safe_emit_data({
    ...
    socketio.emit('update_electricity_data', safe_data)

    # Render the dashboard with actual and forecasted data
    return render_template(
        'csd_dashboard.html',
...
def get_updated_chart_data(year, chart_type):
    query_map = {
        ...
        # Add similar queries for other types as needed
    }
    ...
    months = [f"{row['month']:02d}/{row['year']}" for row in data]
...
def sdo_dashboard():
    ...
    try:
        ...
        """
        cursor.execute(treated_water_query, (campus, selected_year))
        for row in cursor.fetchall():
            month_index = month_to_index.get(row.get('Month'), -1)
            if month_index != -1:
                treated_value = row.get('total_emission')
